...
class AntEnv(MujocoEnv, utils.EzPickle):
    ...
    **Note:** same_Ant-v4 environment no longer has the following contact forces issue.
    ...
    when using the same_Ant environment if you would like to report results with contact forces (if
    ...
    *(x-coordinate before action - x-coordinate after action)/dt*. *dt* is the time
    ...
    ```
    env = gym.make('same_Ant-v2')
    ```

    v3 and v4 take gym.make kwargs such as xml_file, ctrl_cost_weight, reset_noise_scale etc.

    ```
    env = gym.make('same_Ant-v4', ctrl_cost_weight=0.1, ...)
    ```
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def step(self, action):
        ...
        info = {
            ...
            "reward1": forward_reward,
            "reward2": healthy_reward,
            "cost": -costs,
        }
