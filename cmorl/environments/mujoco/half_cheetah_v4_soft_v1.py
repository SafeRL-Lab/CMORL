...
class HalfCheetah_Soft_v1_Env(MujocoEnv, utils.EzPickle):
    """
    ### Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def step(self, action):
        ...
        forward_reward = -np.square(x_velocity - self._goal_vel)
        ...
        ## changes begin
        height = -0.0
        cost = -np.square(observation[0] - height)
        info = {
            "x_position": x_position_after,
            "x_velocity": x_velocity,
            "reward_l1": -np.abs(x_velocity - self._goal_vel), # reward objective one
            "reward_run": forward_reward,
            "reward_ctrl": -ctrl_cost, # reward objective two
            "reward1": -np.abs(x_velocity - self._goal_vel),
            "reward2": -ctrl_cost,
            "reward3": -np.abs(observation[0] - height),
            "cost": -np.abs(observation[0] - height),
        }
