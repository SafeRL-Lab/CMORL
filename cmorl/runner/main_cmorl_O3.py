...
from cmorl.algorithms.trpo import Cagrad_all  # all tasks>2
from cmorl.algorithms.models import Policy, Value
from cmorl.algorithms.replay_memory_O3 import Memory
from cmorl.utils.running_state import ZFilter
from cmorl.utils.utils import normal_entropy, normal_log_density, get_flat_params_from, set_flat_params_to, \
    get_flat_grad_from
...
def update_params(batch, cost_batch, i_episode):
    ...
    rewards3 = torch.Tensor(batch.reward3)
    ...
    values3 = value_net3(Variable(states))
    ...
    returns3 = torch.Tensor(actions.size(0), 1)
    deltas3 = torch.Tensor(actions.size(0), 1)
    advantages3 = torch.Tensor(actions.size(0), 1)
    ...
    prev_value3 = 0
    prev_advantage3 = 0
    for i in reversed(range(rewards3.size(0))):
        returns3[i] = rewards3[i] + args.gamma * prev_return3 * masks[i]
        deltas3[i] = rewards3[i] + args.gamma * prev_value3 * masks[i] - values3.data[i]
        advantages3[i] = deltas3[i] + args.gamma * args.tau * prev_advantage3 * masks[i]

        prev_return3 = returns3[i, 0]
        prev_value3 = values3.data[i, 0]
        prev_advantage3 = advantages3[i, 0]
    ...
    def get_value_loss3(flat_params):
        set_flat_params_to(value_net3, torch.Tensor(flat_params))
        for param in value_net3.parameters():
            if param.grad is not None:
                param.grad.data.fill_(0)
        values_3 = value_net3(Variable(states))
        value_loss3 = (values_3 - targets3).pow(2).mean()
        # weight decay
        for param in value_net3.parameters():
            value_loss3 += param.pow(2).sum() * args.l2_reg
        value_loss3.backward()
        return (value_loss3.data.double().numpy(), get_flat_grad_from(value_net3).data.double().numpy())

    if cost_batch >= -0.3 or i_episode <= args.start_safety:
        print("work!")
        ...
        flat_params3, _, opt_info = scipy.optimize.fmin_l_bfgs_b(get_value_loss3,
                                                                 get_flat_params_from(value_net3).double().numpy(),
                                                                 maxiter=25)
        set_flat_params_to(value_net3, torch.Tensor(flat_params3))

        advantages = (advantages - advantages.mean()) / advantages.std()
        advantages2 = (advantages2 - advantages2.mean()) / advantages2.std()
        advantages3 = (advantages3 - advantages3.mean()) / advantages3.std()
        ...
        def get_loss3(volatile=False):
            if volatile:
                ...
                action_means, action_log_stds, action_stds = policy_net(Variable(states))
            log_prob = normal_log_density(Variable(actions), action_means, action_log_stds, action_stds)
            action_loss3 = -Variable(advantages3) * torch.exp(log_prob - Variable(fixed_log_prob))
            return action_loss3.mean()
        ...
        trpo_step(policy_net, get_loss3, get_kl, 0.05, args.damping)
        grads3 = get_flat_params_from(policy_net) - prev_policy_net_data
        cagrad_all_tasks = Cagrad_all(c=0.5)
        add_dem_grads1 = grads1.unsqueeze(0)
        add_dem_grads2 = grads2.unsqueeze(0)
        add_dem_grads3 = grads3.unsqueeze(0)
        grad_vec = torch.cat((add_dem_grads1, add_dem_grads2, add_dem_grads3))
        final_grad = cagrad_all_tasks.cagrad(grad_vec, len(grad_vec))
        set_flat_params_to(policy_net, prev_policy_net_data + final_grad)
    else:
        flat_params, _, opt_info = scipy.optimize.fmin_l_bfgs_b(get_cost_loss,
                                                                get_flat_params_from(cost_net).double().numpy(),
                                                                maxiter=25)
        set_flat_params_to(cost_net, torch.Tensor(flat_params))
...
for i_episode in count(1):
    ...
    while num_steps < EPISODE_LENGTH * EPISODE_PER_BATCH:
        if args.env_name == "Humanoid-dm":
            ...
            for t in range(EPISODE_LENGTH):  # Don't infinite loop while learning
                ...
                reward, reward2, reward3, cost = reward
                reward_sum += reward
                reward_sum2 += reward2
                reward_sum3 += reward3
                cost_sum += cost
                ...
                if t == EPISODE_LENGTH - 1:
                    mask = 0
                memory.push(state, np.array([action]), mask, next_state, reward, reward2, reward3,
                            cost)
                state = next_state
        else:
            ...
            for t in range(EPISODE_LENGTH):  # Don't infinite loop while learning
                ...
                reward_data3.append(info["reward3"])
    ...
    reward_datas3.append(reward_data3)
    cost_datas.append(cost_data)

    np.save("cmorl/data/HalfCheetah_v4_Soft_v1/reward.npy", np.array(reward_datas))
    np.save("cmorl/data/HalfCheetah_v4_Soft_v1/reward2.npy", np.array(reward_datas2))
    np.save("cmorl/data/HalfCheetah_v4_Soft_v1/reward3.npy", np.array(reward_datas3))
    np.save("cmorl/data/HalfCheetah_v4_Soft_v1/cost.npy", np.array(cost_datas))

    batch = memory.sample()
    update_params(batch, cost_step / num_steps, i_episode)
    if i_episode % args.log_interval == 0:
        print(
            f'Epoch {i_episode}\tAverage reward1 {reward_batch / num_steps:.7f}\tAverage reward2 {reward_batch2 / num_steps:.7f}\tAverage reward3 {reward_batch3 / num_steps:.7f}\t Average cost {-cost_step / num_steps:.7f}')
