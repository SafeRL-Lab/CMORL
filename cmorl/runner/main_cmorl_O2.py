...
from cmorl.algorithms.replay_memory_O2 import Memory
...
if args.env_name in ["Walker-dm", "Humanoid-dm"]:
...
def update_params(batch, cost_batch, i_episode):
    ...
    if cost_batch >= -args.safety_bound or i_episode <= args.start_safety:
        ...
        if args.with_momentum=="False":
            ...
            cr_mogm = CR_MOGM(alpha=0.5, lam=0.1)
            final_grad = cr_mogm.cr_mogm_grad(grads1, grads2, c=0.5)
...
for i_episode in count(1):
    ...
    # np.save("cmorl/data/"+args.env_name+"/reward.npy", np.array(reward_datas))
    # np.save("cmorl/data/"+args.env_name+"/reward2.npy", np.array(reward_datas2))
    # np.save("cmorl/data/"+args.env_name+"/cost.npy", np.array(cost_datas))

    batch = memory.sample()
    update_params(batch, cost_step / num_steps, i_episode)
    if i_episode % args.log_interval == 0:
        print(
            f'Epoch {i_episode}\tAverage reward1 {reward_batch / num_steps:.7f}\tAverage reward2 {reward_batch2 / num_steps:.7f}\t Average cost {-cost_step / num_steps:.7f}')
